...
```{r}
ggplot(data=stimmen) + geom_boxplot(aes(x=subject, y=frequency)) 
```

Machen Sie das gleiche für `scenario`. 

```{r}
ggplot(data=stimmen) + geom_boxplot(aes(x=scenario, y=frequency))
```

Gibt es mehr Varianz zwischen Szenarien oder zwischen Versuchspersonen?

Antwort (für mich selbst): Zwischen den Versuchspersonen gibt es mehr Varianz als zwischen den verschiedenen Szenarian. Tatsächlich scheint bei den Scenarien die Varianz innerhalb der Gruppen größer zu sein als zwischen den Gruppen.
...
Antwort(für mich selbst): Nein, sie haben sich nicht geändert. Es kommen offenbar lediglich die zusätzlichen Werte für attitude dazu.
...
Antwort (für mich selbst): Ja, können wir. Beide Modelle unterscheiden sich nicht signifikant, weshalb wir das einfachere Modell wählen sollten.
...
Wir sehen, dass das Intercept für jedes `subject` und `scenario` unterschiedlich ist, aber die Koeffizienten für die Anstige (`genderM` und `attitudepol`) überall gleich sind und zwar die gleichen, die es als fixe Effekte gibt!  Bei dem Modell mit Anstieg in der RE-Struktur gibt es einen Anstieg für jedes `subject` und `scenario`.
...
# Lizenz
Dieses Werk ist lizenziert unter einer CC-BY-NC-SA Lizenz. Diese Datei darf als Beispiel weiterverwendet werden. 
