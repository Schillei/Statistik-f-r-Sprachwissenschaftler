...
Nein, besonders hinsichtlich der Bedingungen DD und ED erkennt man Unterschiede. Die Streuung für diese Bedingungen scheinen by subject größer zu sein als by item. Im Vergleich zu den Single-Trial Daten, sehen die Varianzen der by-item und by-subject Daten über die Bedingungen hinweg heterogener aus.
...
```{r}
priming.f2 <- ezANOVA(priming, dv=.(RT), wid=.(item), within=.(prime,target), detailed=TRUE)
```
...
```{r, results='asis'}
priming.f1.deutsch.target <- ezANOVA(subset(priming, target == "D"), dv=.(RT), wid=.(subj), within=.(prime), detailed=TRUE
                                      )
print(xtable(priming.f1.deutsch.target$ANOVA, display=c("s","s","d","d","f","f","f","fg","s","g"), digits=c(0,0,0,0,2,2,2,2,0,2)),type="html",include.rownames=FALSE)

```


# Lizenz
Dieses Werk ist lizenziert unter einer CC-BY-NC-SA Lizenz. Die Datei darf nur zu Prüfungszwecken verwendet werden.