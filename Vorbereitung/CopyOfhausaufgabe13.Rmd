...
DENSITY
```{r}
ggplot(data=priming) + geom_density(aes(x=RT, color=cond, fill=cond), alpha=0.5) 
```
JITTER
```{r}
...
VIOLIN
```{r}
ggplot(data=priming) + geom_violin(aes(x=cond, y=RT, color=cond, fill=cond), alpha=0.7)
```
BOXPLOT
```{r}
...
```{r, results='asis'}
priming.by.subject.wide1 <- dcast(data=priming.by.subject, subj ~ cond, value.var="RT")
print(xtable(priming.by.subject.wide1))
```
 
Mit Bedingungen als Zeilen und Versuchspersonen als Spalten:
```{r, results='asis'}
priming.by.subject.wide2 <- dcast(data=priming.by.subject, cond ~ subj, value.var="RT")
...
```{r}
ggplot(data=priming.by.subject) + geom_density(aes(x=RT, color=cond, fill=cond), alpha=0.5)
...
## Subject-Analyse
DATEN DER EINZELNEN SUBJECTS PLOTTEN
```{r}
ggplot(data=priming) 
```

```{r}
priming.f1 <- ezANOVA(CODE)
```
...
## Item-Analyse
```{r}
DATEN FÜR ALLE SUBJECTS EINZELN PLOTTEN
```
